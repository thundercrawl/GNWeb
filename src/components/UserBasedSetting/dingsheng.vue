...
import dateClum from "../common/dateClum"
 ...
 importExcel:function()
      ...
      exportExcel:function(date)
    {
       ...
       if(date == '')
       {
            window.location.href  = Config.serviceUrl+"/ExportExcelAllByName?name=dingsheng&date=ALL"
       }
       else
       {
           window.location.href =Config.serviceUrl+"/ExportExcelAllByName?name=dingsheng&"+"date="+this.$global.dateColumn().yearvalue+"-"+this.$global.dateColumn().monthvalue
       }
    ...
    addCartypeSubmit:function()
      ...
      deleteItem:function(index,row)
      {
        ...
        this.$confirm('确认删除该记录吗?', '提示', {type: 'warning'}).then(() => {
          ...
          let params = {}
          params.tid = row.tid;
           this.$http.post(this.$global.remote().dingshengDelete, params, response => {
