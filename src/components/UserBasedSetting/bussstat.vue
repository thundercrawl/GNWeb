...
import Config from '../../config';
  export default{
      ...
      exportExcel:function(date)
    {
       ...
       if(date == '')
       {
            window.location.href  = Config.serviceUrl+"/ExportExcelAllByName?name=bussstat&date=ALL"
       }
       else
       {
           window.location.href =Config.serviceUrl+"/ExportExcelAllByName?name=bussstat&"+"date="+this.$global.dateColumn().yearvalue+"-"+this.$global.dateColumn().monthvalue
       }
      ...
      search(){
        ...
        that.loading = true;
        that.$http.get(this.$global.remote().bussstatListByMonth, params, response => {
          console.log("get bussstat list")
          if (that.$tools.isNotEmpty(response.result)) {
           ...
           // that.result = response.result;
            ...
            that.total =2
            that.result = [];
            that.result[0] = Object.assign({}, response.result.kunpengitem)
            console.log(that.result[0])
            that.result[1] = Object.assign({}, response.result.dingshengitem)
            that.result[0].company = "坤鹏业务";
            that.result[1].company = "鼎盛业务";
            /*
