        ...
        <el-dialog
            ...
            <el-form label-width="80px" :model="form" :rules="rules" ref="form" size="small" :inline="true">
                ...
                <el-checkbox-group v-model="checkList">
                    ...
                    <el-checkbox label="导入Excel"></el-checkbox>
    ...
    export default {
        ...
        methods: {
            ...
            onUpdate() {
                ...
                let userName = this.multipleSelection[0].userName;
                
                if(tools.isEmpty(this.$global.userPermissionNotRole)) {
                console.log("user permission set not exists and get from server")
                this.$http.get(this.$global.remote().userPermissionNotRoleBased, {userName:userName}, response => {
                    //this.form = response.result;
                    
                     let userPermissions = response.result
                     this.checkList = [];
                     console.log("user permissions:"+userPermissions);
                     console.log(userPermissions.indexOf("/expenseVelocity/read"))
                    if(userPermissions.indexOf("/UserMgr/delete")>=0)
                        this.checkList.push("添加删除成员")
                    if(userPermissions.indexOf("/expenseVelocity/read")>=0)
                        this.checkList.push("查看信息")
                    if(userPermissions.indexOf("/expenseVelocity/edit")>=0)
                        this.checkList.push("编辑信息")
                     if(userPermissions.indexOf("/expenseVelocity/excelI")>=0)
                        this.checkList.push("导入Excel")
                    if(userPermissions.indexOf("/expenseVelocity/excelE")>=0)
                        this.checkList.push("导出Excel")
            ...
            convertPermssionNotRole(name){
                console.log("convert name:"+name)
            }
            ...
            handleSubmit(){
                ...
                param.permissionsNotRole = this.checkList;
        ...
        mounted(){
            let userPermissions = this.$global.getUserPermissionsNotRole();
            console.log("load current user's permissionnotrole:"+userPermissions)
            /* role based code
            ...
            */
           console.log(userPermissions.indexOf("/UserMgr/read"))
           this.buttons.query = userPermissions.indexOf("/UserMgr/read") >= 0;
            this.buttons.add = userPermissions.indexOf("/UserMgr/edit") >= 0;
            this.buttons.update = userPermissions.indexOf("/UserMgr/edit") >= 0;
            this.buttons.delete = userPermissions.indexOf("/UserMgr/delete") >= 0;
            this.buttons.distribute = userPermissions.indexOf(this.$global.remote().distributeRole) >= 0;
            this.checkList = ["查看信息","导出Excel"]
        }
    }
